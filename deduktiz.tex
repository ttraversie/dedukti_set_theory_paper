...
The IZmod theory is an extension of IZst set theory with an encoding of pointed graphs, as the axioms of IZst are provable in IZmod\cite[see Section 3.3]{zermodulo}.
...
Inductive sets are used in the axiom of infinity. The previous rule states that $c$ is inductive if (i) it contains the empty set and (ii) if $a \in c$ then $a \cup \{a\} \in c$. Here we use \texttt{pair a a} to form the singleton $\{a\}$.
