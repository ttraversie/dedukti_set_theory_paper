...
\title{\textbf{Dedukti: an implementation of set theory with pointed graphs}}
\date{}
...
We have implemented in \dedukti ~a version of set theory -- IZst -- that correponds to Zermelo set theory, with the Strong Extensionality axiom and the Transitive Closure axiom. To do so, we have adapted the work by G. Dowek and A. Miquel \cite{zermodulo} from \textit{Deduction modulo theory} to \lpcm ~and have encoded set with a structure of pointed graphs of IZmod theory. We have \textit{formally} written all the proofs of the lemmas allowing us to implement set theory in \dedukti.
